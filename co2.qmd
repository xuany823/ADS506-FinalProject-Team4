...
## 4. Forecasting and Forecast Accuracy on Test Set
...
## 4.5 Rolling Origin Cross-validation

### Compare models the first 10 years of the CO2 training data (no CV)
...
### Rolling-Origin Cross-Validation (10-year initial window, 1-year increments)
...
co2_cv_fit <- co2_cv |>
  model(log_tslm  = TSLM(log(average) ~ trend() + season()),
        log_ets   = ETS(log(average)),
        log_arima = ARIMA(log(average), stepwise = TRUE, approximation = TRUE),
        snaive    = SNAIVE(average)) 
